...
docker build . --tag local/operator:${TAG} --load

echo "Loading image into kind cluster ${CLUSTER_NAME}..."
kind load --name "${CLUSTER_NAME}" docker-image local/operator:${TAG}
...
kubectl -n "${NAMESPACE}" set image deployment/rossoctl-controller-manager manager=local/operator:${TAG}

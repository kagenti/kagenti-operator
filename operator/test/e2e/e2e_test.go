...
func controllerNS() string {
	...
	return "rossoctl-operator-system"
}
...
func controllerDeploy() string {
	...
	return "rossoctl-operator-controller-manager"
}
...
func webhookServiceName() string {
	...
	return "rossoctl-operator-webhook-service"
}

var namespace = controllerNS()

// serviceAccountName created for the project
const serviceAccountName = "rossoctl-operator-controller-manager"

// metricsServiceName is the name of the metrics service of the project
const metricsServiceName = "rossoctl-operator-controller-manager-metrics-service"

// metricsRoleBindingName is the name of the RBAC that will be created to allow get the metrics data
const metricsRoleBindingName = "rossoctl-operator-metrics-binding"
	...
	Context("Manager", func() {
		...
		It("should ensure the metrics endpoint is serving metrics", func() {
			...
			cmd := exec.Command("kubectl", "create", "clusterrolebinding", metricsRoleBindingName,
				"--clusterrole=rossoctl-operator-metrics-reader",
...
var _ = Describe("AuthBridge Injection E2E", Ordered, func() {
	...
	Context("Sidecar injection", Ordered, func() {
		It("should inject envoy-proxy + proxy-init with bundled spiffe-helper", func() {
			...
			By("waiting for operator to apply rossoctl.io/type label via AgentRuntime")
			Eventually(func(g Gomega) {
				typeLabel, labelErr := utils.KubectlGetJsonpath("deployment", "authbridge-agent", authBridgeTestNamespace,
					"{.metadata.labels.rossoctl\\.io/type}")
				g.Expect(labelErr).NotTo(HaveOccurred())
				g.Expect(typeLabel).To(Equal("agent"))
			}, 1*time.Minute, 2*time.Second).Should(Succeed())

			By("waiting for pod template to have rossoctl.io/type label (triggers rolling update)")
			Eventually(func(g Gomega) {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(containers).NotTo(ContainSubstring("rossoctl-client-registration"))
			}).Should(Succeed())
		...
		It("should not duplicate sidecars on pod recreation (idempotency)", func() {
			...
			Eventually(func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "pods",
					"-l", "app.kubernetes.io/name=authbridge-agent,rossoctl.io/type=agent",
	...
	Context("Injection opt-out", func() {
		It("should not inject when rossoctl.io/inject=disabled", func() {
			var err error
			...
			Eventually(func(g Gomega) {
				typeLabel, labelErr := utils.KubectlGetJsonpath("deployment", "authbridge-disabled-agent",
					authBridgeTestNamespace, "{.metadata.labels.rossoctl\\.io/type}")
				g.Expect(labelErr).NotTo(HaveOccurred())
				g.Expect(typeLabel).To(Equal("agent"))
			}, 1*time.Minute, 2*time.Second).Should(Succeed())
...
var _ = Describe("AgentCard E2E", Ordered, func() {
	...
	Context("Without signature verification", Ordered, func() {
		...
		It("should not create AgentCard for workload without protocol label", func() {
			...
			By("waiting for operator to apply rossoctl.io/type label")
			Eventually(func(g Gomega) {
		...
		It("should auto-create AgentCard for labelled workload", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(managedBy).To(Equal("rossoctl-operator"))
			}).Should(Succeed())
	...
	Context("With signature verification", Ordered, func() {
		...
		BeforeAll(func() {
			...
			origArgs, err = utils.PatchControllerArgs(controllerNamespace, controllerDeployment, []string{
				...
				"ROSSOCTL_SPIRE_TRUST_DOMAIN": "example.org",
			})
			Expect(err).NotTo(HaveOccurred())
		})

		AfterAll(func() {
			By("removing ROSSOCTL_SPIRE_TRUST_DOMAIN env var")
			cmd := exec.Command("kubectl", "set", "env", "deployment/"+controllerDeployment,
				"-n", controllerNamespace, "ROSSOCTL_SPIRE_TRUST_DOMAIN-")
			_, _ = utils.Run(cmd)
...
`)
		...
		By("ensuring rossoctl-system namespace exists")
		cmd = exec.Command("kubectl", "create", "ns", "rossoctl-system")
		_, _ = utils.Run(cmd) // ignore error; namespace may already exist

		By("creating cluster defaults ConfigMap")
		_, err = utils.KubectlApplyStdin(runtimeClusterDefaultsConfigMapFixture(), "rossoctl-system")
		Expect(err).NotTo(HaveOccurred())
	...
	AfterAll(func() {
		...
		cmd = exec.Command("kubectl", "delete", "configmap", "rossoctl-platform-config",
			"-n", "rossoctl-system", "--ignore-not-found")
		_, _ = utils.Run(cmd)
	...
	Context("Agent lifecycle", Ordered, func() {
		It("should apply labels and config-hash to target Deployment", func() {
			...
			By("verifying rossoctl.io/type=agent on workload metadata")
			Eventually(func(g Gomega) {
				typeLabel, err := utils.KubectlGetJsonpath("deployment", "runtime-agent-target",
					agentRuntimeTestNamespace, "{.metadata.labels['rossoctl\\.io/type']}")
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(typeLabel).To(Equal("agent"))
			}).Should(Succeed())
			...
			By("verifying rossoctl.io/type=agent on pod template")
			Eventually(func(g Gomega) {
		...
		It("should clean up on deletion", func() {
			...
			By("verifying rossoctl.io/type label is removed from workload metadata")
			Eventually(func(g Gomega) {
				typeLabel, err := utils.KubectlGetJsonpath("deployment", "runtime-agent-target",
					agentRuntimeTestNamespace, "{.metadata.labels['rossoctl\\.io/type']}")
				g.Expect(err).NotTo(HaveOccurred())
			...
			By("verifying rossoctl.io/type label is removed from PodTemplateSpec")
			Eventually(func(g Gomega) {
				typeLabel, err := utils.KubectlGetJsonpath("deployment", "runtime-agent-target",
					agentRuntimeTestNamespace, "{.spec.template.metadata.labels['rossoctl\\.io/type']}")
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(typeLabel).To(BeEmpty())
			}).Should(Succeed())
	...
	Context("Tool type", func() {
		It("should apply rossoctl.io/type=tool label", func() {
			...
			By("verifying rossoctl.io/type=tool on workload metadata")
			Eventually(func(g Gomega) {
	...
	Context("StatefulSet target", func() {
		It("should apply labels and config-hash to a StatefulSet workload", func() {
			...
			By("verifying rossoctl.io/type=agent on StatefulSet metadata")
			Eventually(func(g Gomega) {
...
`)
		...
		By("setting ROSSOCTL_SPIRE_TRUST_DOMAIN env var")
		envCmd := exec.Command("kubectl", "set", "env", "deployment/"+controllerDeployment,
			"-n", controllerNamespace, "ROSSOCTL_SPIRE_TRUST_DOMAIN=example.org")
	...
	It("should apply labels to workload when AgentRuntime is created", func() {
		...
		By("verifying rossoctl.io/type=agent on deployment metadata")
		Eventually(func(g Gomega) {
			typeLabel, err := utils.KubectlGetJsonpath("deployment", "combined-agent",
				combinedTestNamespace, "{.metadata.labels['rossoctl\\.io/type']}")
			g.Expect(err).NotTo(HaveOccurred())
			g.Expect(typeLabel).To(Equal("agent"))
		}).Should(Succeed())
		...
		By("verifying protocol.rossoctl.io/a2a label preserved")
		Eventually(func(g Gomega) {
			labelsJSON, err := utils.KubectlGetJsonpath("deployment", "combined-agent",
				combinedTestNamespace, "{.metadata.labels}")
			g.Expect(err).NotTo(HaveOccurred())
			g.Expect(labelsJSON).To(ContainSubstring("protocol.rossoctl.io/a2a"))
		}).Should(Succeed())
	...
	It("should clean up on AgentRuntime deletion and stop injection", func() {
		...
		By("verifying rossoctl.io/type label removed from workload metadata")
		Eventually(func(g Gomega) {
			typeLabel, err := utils.KubectlGetJsonpath("deployment", "combined-agent",
				combinedTestNamespace, "{.metadata.labels['rossoctl\\.io/type']}")
			g.Expect(err).NotTo(HaveOccurred())
		...
		By("verifying rossoctl.io/type label removed from PodTemplateSpec")
		Eventually(func(g Gomega) {
			typeLabel, err := utils.KubectlGetJsonpath("deployment", "combined-agent",
				combinedTestNamespace, "{.spec.template.metadata.labels['rossoctl\\.io/type']}")
			g.Expect(err).NotTo(HaveOccurred())
			g.Expect(typeLabel).To(BeEmpty())
		}).Should(Succeed())
...
`)
		...
		cleanupCmd := exec.Command("kubectl", "delete", "configmap", "rossoctl-feature-gates",
	...
	AfterAll(func() {
		...
		cmd = exec.Command("kubectl", "delete", "configmap", "rossoctl-feature-gates",
			"-n", controllerNamespace, "--ignore-not-found")
		_, _ = utils.Run(cmd)
	...
	Context("Feature gate enabled", Ordered, func() {
		...
		It("should update linkedSkills when annotation changes", func() {
			...
			cmd := exec.Command("kubectl", "annotate", "--overwrite",
				...
				"-n", skillDiscoveryTestNamespace,
				`rossoctl.io/skills=["summarizer","translator","openshift-review"]`)
			_, err := utils.Run(cmd)
			Expect(err).NotTo(HaveOccurred())
		...
		It("should degrade gracefully with malformed skills annotation", func() {
			By("setting a malformed rossoctl.io/skills annotation")
			cmd := exec.Command("kubectl", "annotate", "--overwrite",
				"deployment", "skill-discovery-agent",
				"-n", skillDiscoveryTestNamespace,
				`rossoctl.io/skills=not-valid-json`)
			_, err := utils.Run(cmd)
			Expect(err).NotTo(HaveOccurred())
			...
			Eventually(func(g Gomega) {
				...
				output, err := utils.Run(cmd)
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(output).To(ContainSubstring("Failed to parse rossoctl.io/skills annotation"))
			}).Should(Succeed())
			...
			cmd = exec.Command("kubectl", "annotate", "--overwrite",
				"deployment", "skill-discovery-agent",
				"-n", skillDiscoveryTestNamespace,
				`rossoctl.io/skills=["summarizer","openshift-review"]`)
			_, err = utils.Run(cmd)
			Expect(err).NotTo(HaveOccurred())
		})
...
var _ = Describe("Istio Mesh Enrollment E2E", Ordered, func() {
	...
	BeforeAll(func() {
		...
		cmd = exec.Command("kubectl", "annotate", "ns", istioMeshOptOutTestNamespace,
			"rossoctl.io/istio-mesh=disabled")
		_, err = utils.Run(cmd)
		Expect(err).NotTo(HaveOccurred())

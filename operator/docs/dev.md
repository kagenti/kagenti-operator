...
This guide covers everything you need to know to develop, test, and contribute to the Rossoctl Operator.
...
```bash
git clone https://github.com/rossoctl/operator.git
...
helm uninstall rossoctl-operator -n rossoctl-system
...
kind create cluster --name rossoctl
...
```bash
# Set image name
export IMG=myregistry/operator:v1.0.0
...
helm install rossoctl-operator ../charts/operator \
  ...
  --create-namespace \
  --set controllerManager.container.image.repository=myregistry/operator \
...
```bash
kubectl port-forward -n rossoctl-system deployment/rossoctl-operator-controller-manager 2345:2345
```
...
```bash
# Controller logs
kubectl logs -n rossoctl-system -l control-plane=controller-manager -f
...
2. Verify the backing workload has `rossoctl.io/type=agent` label
...
```bash
# View all CRDs
kubectl get crds | grep agent.rossoctl.dev

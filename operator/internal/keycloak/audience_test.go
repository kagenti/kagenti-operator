...
func TestEnsureAudienceScope(t *testing.T) {
	var listScopesCalls, postScopeCalls, postMapperCalls, putRealmCalls, listRossoctlCalls, putClientCalls int
	...
	srv = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case path == "/admin/realms/rossoctl/client-scopes" && r.Method == http.MethodGet:
			listScopesCalls++
			_ = json.NewEncoder(w).Encode([]clientScopeListItem{})
		case path == "/admin/realms/rossoctl/client-scopes" && r.Method == http.MethodPost:
			postScopeCalls++
			w.Header().Set("Location", srv.URL+"/admin/realms/rossoctl/client-scopes/new-scope-id")
		...
		case path == "/admin/realms/rossoctl/default-default-client-scopes/new-scope-id" && r.Method == http.MethodPut:
			putRealmCalls++
			w.WriteHeader(http.StatusNoContent)
		case strings.HasPrefix(path, "/admin/realms/rossoctl/clients") && r.Method == http.MethodGet && r.URL.Query().Get("clientId") == "rossoctl":
			listRossoctlCalls++
			_ = json.NewEncoder(w).Encode([]map[string]string{{"id": "plat-int", "clientId": "rossoctl"}})
		case path == "/admin/realms/rossoctl/clients/plat-int/default-client-scopes/new-scope-id" && r.Method == http.MethodPut:
			putClientCalls++
			w.WriteHeader(http.StatusNoContent)
		default:
	...
	err = a.EnsureAudienceScope(context.Background(), token, AudienceParams{
		...
		PlatformClientIDs:    []string{"rossoctl"},
	...
	if postScopeCalls != 1 || postMapperCalls != 1 || putRealmCalls != 1 || listRossoctlCalls != 1 || putClientCalls != 1 {
		t.Fatalf("calls scope=%d mapper=%d realm=%d listK=%d putC=%d", postScopeCalls, postMapperCalls, putRealmCalls, listRossoctlCalls, putClientCalls)
	}
}
...
func TestEnsureAudienceScope_AttachesToAgentClient(t *testing.T) {
	...
	srv = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.HasPrefix(path, "/admin/realms/rossoctl/clients") && r.Method == http.MethodGet && r.URL.Query().Get("clientId") == "rossoctl":
			_ = json.NewEncoder(w).Encode([]map[string]string{{"id": "plat-int", "clientId": "rossoctl"}})
		case path == "/admin/realms/rossoctl/clients/agent-uuid-123/default-client-scopes/new-scope-id" && r.Method == http.MethodPut:
			putAgentClientCalls++
			w.WriteHeader(http.StatusNoContent)
		case path == "/admin/realms/rossoctl/clients/plat-int/default-client-scopes/new-scope-id" && r.Method == http.MethodPut:
...
func TestEnsureAudienceScope_UpdatesStaleMapper(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Realm default scope
		case path == "/admin/realms/rossoctl/default-default-client-scopes/scope-123" && r.Method == http.MethodPut:
			w.WriteHeader(http.StatusNoContent)

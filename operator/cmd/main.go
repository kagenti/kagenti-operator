...
import (
	...
	"github.com/rossoctl/operator/internal/bootstrap"
	"github.com/rossoctl/operator/internal/controller"
	"github.com/rossoctl/operator/internal/discovery"
	"github.com/rossoctl/operator/internal/keycloak"
	"github.com/rossoctl/operator/internal/kuadrant"
	"github.com/rossoctl/operator/internal/mlflow"
	"github.com/rossoctl/operator/internal/signature"
	"github.com/rossoctl/operator/internal/tekton"
	webhookconfig "github.com/rossoctl/operator/internal/webhook/config"
	"github.com/rossoctl/operator/internal/webhook/injector"
	webhookv1alpha1 "github.com/rossoctl/operator/internal/webhook/v1alpha1"
...
func getOperatorNamespace() string {
	...
	setupLog.Info("POD_NAMESPACE not set, using default", "default", "rossoctl-system")
	return "rossoctl-system"
}
...
func main() {
	...
	flag.StringVar(&configPath, "config-path", "/etc/rossoctl/config.yaml", "Path to platform config file")
	flag.StringVar(&featureGatesPath, "feature-gates-path",
		"/etc/rossoctl/feature-gates/feature-gates.yaml", "Path to feature gates config file")
	...
	flag.StringVar(&mlflowExperimentName, "mlflow-experiment-name", "rossoctl-traces",
	...
	flag.StringVar(&keycloakRealm, "keycloak-realm", "rossoctl",
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "b7c4ae34.rossoctl.dev",
	})
	if err != nil {
	...
	if keycloakPublicURL == "" {
		keycloakPublicURL = os.Getenv("ROSSOCTL_KEYCLOAK_PUBLIC_URL")
	}
	if keycloakPublicURL == "" {
	...
	if spireTrustDomain == "" {
		spireTrustDomain = os.Getenv("ROSSOCTL_SPIRE_TRUST_DOMAIN")
	}
	if spireTrustDomain == "" {
		spireNS := os.Getenv("ROSSOCTL_SPIRE_NAMESPACE")
		...
		if spireTrustDomain == "" {
			setupLog.Error(errors.New("SPIRE trust domain not available"),
				"spireTrustDomain is required when --require-a2a-signature=true "+
					"(set ROSSOCTL_SPIRE_TRUST_DOMAIN or ensure ZTWIM/spire-bundle is accessible)")
			os.Exit(1)
		}
	...
	if controller.SpireOperandCRDExists(mgr.GetConfig()) {
		if spireTrustDomain == "" {
			...
			spireClusterName := os.Getenv("ROSSOCTL_SPIRE_CLUSTER_NAME")
...
// ConfigMaps. The scoped cache ensures only rossoctl-relevant ConfigMaps are
// watched instead of every ConfigMap cluster-wide.
//
// Three categories are included:
//  1. Cluster-level defaults in rossoctl-system (label selector).
	...
	namespaces := map[string]cache.Config{
		controller.ClusterDefaultsNamespace: {
			LabelSelector: labels.SelectorFromSet(map[string]string{
				"app.kubernetes.io/name": "operator-chart",
			}),
		},
		cache.AllNamespaces: {
			LabelSelector: labels.SelectorFromSet(map[string]string{
